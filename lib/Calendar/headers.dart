...
abstract class ZoomableHeadersWidget<C extends ZoomController>
...
abstract class ZoomableHeadersWidgetState<W extends ZoomableHeadersWidget<C>,
    C extends ZoomController> extends State<W> with ZoomControllerListener {
  /// The zoom controller.
  final C controller;
  ...
  ZoomableHeadersWidgetState(ZoomableHeadersWidget<C> parent)
  ...
  void onZoomFactorChanged(
    ...
    if (widget.inScrollableWidget) {
      ...
      double maxPixels =
          calculateHeight(hourRowHeight) - widgetHeight + widget.dayBarHeight;

      if (hourRowHeight < this.hourRowHeight &&
          controller.verticalScrollController.position.pixels > maxPixels) {
  ...
  bool get shouldScrollToCurrentTime =>
      widget.scrollToCurrentTime && widget.inScrollableWidget;
  ...
  void scrollToCurrentTime() {
    ...
    controller.verticalScrollController.jumpTo(math.min(topOffset,
        controller.verticalScrollController.position.maxScrollExtent));
  }
  ...
  double calculateTopOffset(int hour, [int minute = 0, double hourRowHeight]) =>
      (hour + (minute / 60)) * (hourRowHeight ?? this.hourRowHeight);

  /// Calculates the widget height.
  double calculateHeight([double hourRowHeight]) =>
      calculateTopOffset(24, 0, hourRowHeight);
}
...
class DayBar extends StatelessWidget {
  ...
  DayBar.fromHeadersWidget({
    ...
    DateTime date,
          ...
          backgroundColor:
              parent.dayBarBackgroundColor ?? const Color(0xFFEBEBEB),
        ...
        child: Align(
          ...
          child: Text(
            ...
            style: textStyle ??
                TextStyle(
                  color:
                      Utils.sameDay(date) ? Colors.blue[800] : Colors.black54,
...
class HoursColumn extends StatelessWidget {
  ...
  HoursColumn.fromHeadersWidget({
    @required ZoomableHeadersWidget parent,
          ...
          textStyle: parent.hoursColumnTextStyle ??
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Stack(
          children: List.generate(
            23,
            (hour) => Positioned(
              top: (hour + 1) * hourRowHeight -
                  ((textStyle?.fontSize ?? 14) / 2),
              left: 0,
              right: 0,
              child: Text(
